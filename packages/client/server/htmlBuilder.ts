...
export class HtmlBuilder {
  ...
  private styledComponents = '<//-STYLES-//>';
  private htmlString = '';
  ...
  public renderToString(component: string, modules: any, styledComponents: any) {
    ...
    return this.htmlString
      ...
      .replace(this.styledComponents, styledComponents)
    ...
    return `
            ...
            <html>
                ...
                <body>
                    ...
                    ${process.env.NODE_ENV === 'production' ? this.getAsset('runtime') : ''}
                    ${process.env.NODE_ENV === 'production' ? this.getAsset('vendors') : ''}
                    ${this.chunkPlaceholder}
                    ${process.env.NODE_ENV === 'production' ? this.getAsset('main') : this.buildTag('main.js')}
                </body>
            </html>`;
  }
  ...
  private getAsset(chunkName: string, extension = '.js') {
    let chunks = this.stats && this.stats.assetsByChunkName && this.stats.assetsByChunkName[chunkName];
    if (!chunks) {
